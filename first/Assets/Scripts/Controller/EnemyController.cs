    ...
    void Update()
    {
        if(player != null)
        {
            if (Vector3.Distance(player.position, transform.position) > 1.0f)       // Vector3.Distance <== 거리를 알려주는 함수 
            {
                ...
                rb.MovePosition(transform.position + direction * speed * Time.deltaTime);   //플레이어를 향해서 설정한 speed 속도록 이동
            }

            //포탑 회전 
            Vector3 targetDirection = (player.position - enemyPiovt.transform.position).normalized;  //두 벡터를 빼고 Normlized 하면 방향 값을 알려줌 
