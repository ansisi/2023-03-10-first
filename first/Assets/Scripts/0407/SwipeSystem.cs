...
public class SwipeSystem : MonoBehaviour
{
    ...
    {
        float disX = Mathf.Abs(initialPos.x - finalPos.x);          //Mathf.abs 함수는 절대값을 구하는 함수 (거리를 구해야 하기때문에)
        ...
        if (disX > 0 || disY > 0)                                   // || <- or을 나타내는 것 (&&)
        {
            if(disX > disY)
            {
                ...
                {
                    Character.transform.position += new Vector3(-1.0f, 0.0f, 0.0f);         ///Character.transform.position = Character.transform.position + new Vector3(-1.0f, 0.0f, 0.0f); 
                }
                else
                {
